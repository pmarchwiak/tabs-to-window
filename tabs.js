...
function moveTabIdsToWindow(windowInfo, tabIds) {
...
document.addEventListener("click", (e) => {
  ...
  if (e.target.id === "tabs-right-move-new") {
    callOnActiveTab((activeTab, tabs) => {
      const tabsToRight = tabs.filter((tab) => {
        return tab.index > activeTab.index
      });
      const tabIds = tabsToRight.map((t) => t.id);
      console.log(`found ${tabsToRight.length} tabs to right: ${tabIds}`);

      browser.runtime.sendMessage({
        action: "tabs-move-new",
        tabIds: tabIds
      })
